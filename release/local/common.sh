...
BINARY_NAME="xhttp-box"

INSTALL_BIN_PATH="/usr/local/bin"
INSTALL_CONFIG_PATH="/usr/local/etc/xhttp-box"
INSTALL_DATA_PATH="/var/lib/xhttp-box"
...
build_xhttp_box() {
    ...
    echo "Building xhttp-box with tags: $tags"
    cd "$PROJECT_DIR"
...
setup_systemd() {
    ...
    sudo cp "$SCRIPT_DIR/xhttp-box.service" "$SYSTEMD_SERVICE_PATH/"
    sudo systemctl daemon-reload
}

stop_service() {
    if systemctl is-active --quiet xhttp-box; then
...
start_service() {
    echo "Starting xhttp-box service"
    sudo systemctl start xhttp-box
}

restart_service() {
    echo "Restarting xhttp-box service"
    sudo systemctl restart xhttp-box
}
